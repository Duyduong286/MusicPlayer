...
public class AlbumsAdapter extends SelectableAdapter<AlbumsAdapter.ViewHolder> implements RecyclerViewFastScroller.OnPopupTextUpdate {
    ...
    @Override
    public void onBindViewHolder(@NonNull AlbumsAdapter.ViewHolder holder, @SuppressLint("RecyclerView") final int position) {

        holder.albums_album_title.setText(albums.get(position).getAlbum());

        if (isSelected(position)) {
            holder.itemView.setBackgroundColor(context.getResources().getColor(R.color.bluec1));
            holder.albums_popup_menu.setClickable(false);
        }
        else {
            holder.itemView.setBackgroundColor(context.getResources().getColor(R.color.colorBG));
        ...
        Glide.with(holder.albums_album_art).load(albums.get(position).getAlbumArt()).apply(new RequestOptions().skipMemoryCache(true))
                .centerCrop().error(R.drawable.ic_artist_blue).into(holder.albums_album_art);
