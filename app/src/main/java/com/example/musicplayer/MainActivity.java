...
public class MainActivity extends AppCompatActivity {
        ...
        new Runnable() {
            ...
            public void run() {
                ...
                mAppBarConfiguration = new AppBarConfiguration.Builder(
                        R.id.nav_folders, R.id.nav_songs, R.id.nav_playlists, R.id.nav_artists, R.id.nav_albums, R.id.nav_home)
    ...
    protected void onResume() {
        ...
        navigationView.getMenu().getItem(0).setVisible(preferences.getBoolean("HOME", true));
        navigationView.getMenu().getItem(1).setVisible(preferences.getBoolean("SONGS", true));
        navigationView.getMenu().getItem(2).setVisible(preferences.getBoolean("ALBUMS", true));
        navigationView.getMenu().getItem(3).setVisible(preferences.getBoolean("ARTISTS", true));
        navigationView.getMenu().getItem(4).setVisible(preferences.getBoolean("FOLDERS", true));
        navigationView.getMenu().getItem(5).setVisible(preferences.getBoolean("PLAYLISTS", true));
        ...
        new Runnable() {
            ...
            public void run() {
                ...
                if (MediaControllerCompat.getMediaController(MainActivity.this).getPlaybackState().getState() == PlaybackStateCompat.STATE_PLAYING){
                    ...
                    control_iv_play.setImageResource(R.drawable.ic_pause);
                }
                else {
    ...
    public void onHomeClicked(MenuItem item) {
        NavHostFragment.findNavController(getSupportFragmentManager().findFragmentById(R.id.nav_host_fragment)).navigate(R.id.nav_home);
        drawer.closeDrawer(GravityCompat.START);
    ...
    private void playAudioFromFile(Uri fileUri){
        ...
        try {
            ...
            else throw new Exception("Loi cursor");
            if (cursor != null) {
                cursor.close();
            }
        }catch (Exception e){
